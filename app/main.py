...
from monitoring.judge import DEFAULT_JUDGE_MODEL, judge_or_none
...
def get_resources():
    ...
    init_db(log_conn)
    return client, model, judge_model, tools, log_conn


client, model, judge_model, tools, log_conn = get_resources()
    ...
    with st.chat_message(message["role"]):
        ...
        if message["role"] == "assistant":
            ...
            if message.get("verdict"):
                st.caption(format_verdict(message["verdict"]))
            st.feedback(
                "thumbs",
...
if question := st.chat_input("Ask about the 2022 World Cup"):
    ...
    with st.chat_message("assistant"):
        ...
        st.markdown(answer_text)  # paint the answer before waiting on the judge
        st.session_state.history = history
        cost = cost_from_metadata(model, metadata)
        conversation_id = save_conversation(
            log_conn, question, answer_text, metadata, cost
        )
        with st.spinner("Scoring relevance..."):
            verdict = judge_or_none(question, answer, client=client, model=judge_model)
